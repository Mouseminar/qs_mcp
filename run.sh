...
# 检查数据文件是否存在
if [ -f "qs_cleaned.csv" ]; then
    echo "✓ 数据文件已找到" >&2
    export QS_CSV_PATH="/code/qs_cleaned.csv"
else
    echo "⚠️  警告: qs_cleaned.csv 不存在" >&2
fi

echo "========================================" >&2
echo "🚀 启动 QS 世界大学排名 MCP 服务器" >&2
echo "========================================" >&2

# 设置端口环境变量（阿里云函数计算会传入 PORT 环境变量）
# 注意：阿里云函数计算健康检查默认在 9000 端口，必须使用 9000
